...
const handleCompletedFilter = () => {
    ...
    <div className="categories">
        <a href="#" onClick={addTask}><h1 className="allTasksCounter">{AllTasksCount}</h1><p>All Tasks</p><img className="icon_1" src={AllLists} alt="cat1"/></a>
        <a href="#" onClick={handleProgressFilter}><h1 className="inProgress" >{inProgressCount}</h1><p>In Progress</p><img className="icon_3" src={ProgressLists} alt="cat3"/></a>
        <a href="#" onClick={handleCompletedFilter}><h1 className="completed" >{completedCount}</h1><p>Completed</p><img className="icon_4" src={DoneLists} alt="cat4"/></a>
    </div>
    <div className="list">
<div  className="list_template list_new" >
    ...
    <div action="" method="post" className="list_form">
        ...
        />
            ...
            <input type="text" value={newTask} name="input_box_1" placeholder="What are you up to?" className="list_input_1" onChange={ (e) => setNewTask(e.target.value)}/>
            <button className="add_text"><span className="material-symbols-outlined add_text_id" onClick={addTask}><FaPlusCircle/></span></button>
            </div>
            )}
                    ...
                    return (
                        ...
                        <div className="col taskBg">
                            ...
                            <div className="iconsWrap">
                                <span className="icons" text="Completed / Not Completed" onClick={ (e) => markDone(task.id , !task.status) }>
                                    <AiFillCheckCircle />
                                </span>
                                {task.status ? null : (
                                <span className="icons" text="Edit" onClick={ () => {setUpdateData(task)} }>
                                    <MdModeEditOutline/>
                                </span>
                                )}
                                <span className="icons" text="Delete" onClick={ () => deleteTask(task.id, !task.status, task.status)}>
                                    <RiDeleteBin2Fill/>
                                </span>
                            </div>
                        </div>
